...
class Data extends AbstractData
{
    ...
    public function getTermCollection($applyFor)
    {
        return $this->_termFactory->create()
            ->getCollection()
            ->addFieldToFilter('status', \Mageplaza\SeoCrosslinks\Model\Term::STATUS_ENABLED)
    ...
    public function getTermReference($term)
    {
        switch ($term->getReference()) {
            case TargetType::CATEGORY:
                try {
                    $category = $this->categoryRepository->get($term->getRefCategoryId(), $this->storeManager->getStore()->getId());
    ...
    public function replaceKeyword($term, &$content)
    {
        ...
        } else if ($term->getDirection() == Direction::BOTTOM_UP) {
         ...
         */
        $keywordNumber = 0;
        $content = preg_replace_callback($this->getPatternByKeyword($term->getKeyword()), function ($matchesCallback) use ($term, $tmpData, &$keywordNumber) {
