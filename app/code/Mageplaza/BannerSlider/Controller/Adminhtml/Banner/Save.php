...
class Save extends Banner
{
    ...
    public function execute()
    {
        ...
        if ($this->getRequest()->getPost('banner')) {
            ...
            // var_dump( $data );die;
            if( ( !isset( $data['image'])) || isset( $data['image']['delete']) ){
                // if data have no image key [ it si uploading one]
                // if it has request for delete
                $this->imageHelper->uploadImage($data, 'image', Image::TEMPLATE_MEDIA_TYPE_BANNER, $banner->getImage());
            }else{
                // in update remove the key
                unset($data['image']);
            }

            // id data have no image key [ it si uploading one]
            // $this->imageHelper->uploadImage($data, 'image', Image::TEMPLATE_MEDIA_TYPE_BANNER, $banner->getImage());
