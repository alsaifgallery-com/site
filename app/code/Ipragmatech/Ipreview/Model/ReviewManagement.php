    ...
    public function getReviewsList($productId)
    {
        ...
        foreach ($collection as $reviewCollection) {
            ...
            if(is_array($rating) && count($rating)>0){
                 $votesCount = count($rating);
                 $votesTotal = 0;
                foreach ($rating as $vote){
                    $votesTotal = $votesTotal + $vote['value'];
                }
                $overal = $votesTotal/$votesCount;
            }
            $data = [
                "review_id"       => $reviewCollection->getReviewId(),
                "overal"       => (string)$overal,
...
//        }
        $ratings[$rating_id] = $this->getVoteOption($rating_id, $rating_value);
