...
class Save extends \Amasty\Feed\Controller\Adminhtml\Feed
{
    ...
    protected function _save($data)
    {
        ...
        if ($this->getRequest()->getPostValue()) {
            ...
            $deliveryPass = $model->getDeliveryPassword();

            if (!preg_match('/^\*+$/', $deliveryPass) && !empty($deliveryPass)) {
                $model->setDeliveryPassword($this->encryptor->encrypt($deliveryPass));
            }
