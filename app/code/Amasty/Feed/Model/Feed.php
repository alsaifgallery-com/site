...
class Feed extends AbstractModel implements FeedInterface
{
    ...
    /**
     * Don't add \Amasty\Feed\Model\Export\Product to constructor.
     * Use factory instead.
     *
     * Reason:
     * Called the attribute initialization method
     * without reference to any store in the parent constructor of Product class.
     */
    public function afterLoad()
    {
        $this->_export = $this->exportFactory->create(['storeId' => $this->getStoreId()]);
     ...
     * @inheritdoc
     */
    public function export($page, $productIds, $lastPage, $preview = false, $cronGenerated = false)
    {
        if (!isset($this->_export)) {
            $this->_export = $this->exportFactory->create(['storeId' => $this->getStoreId()]);
        }
        $result = $this->_export
            ->setPage($page)
        ...
        $this->setGeneratedAt($cronGenerated ?: date('Y-m-d H:i:s'));
    ...
    protected function _ftpUpload()
    {
        ...
        $this->ftp->open(
            [
                ...
                'password' => $this->encryptor->decrypt($this->getDeliveryPassword()),
