			...
			if($('.active-sticky:not(.header7)').length){
				...
				$(window).scroll(function(){
					...
					if(st > headerHeight){
						...
						// $(".middle-header").css("background-color", "white");
						// $(".action.top-links:before").css("color", "black");
						// $(".wishlist-icon").css("color", "black");
						// $(".action.showcart:before").css("color", "black");
					}else {
						$('.active-sticky').removeClass('start-stk');
						$('.header-area .top-header').slideDown();
						// $(".middle-header").css("background-color", "#be1e2d");
						// $(".action.top-links:before").css("color", "white");
						// $(".wishlist-icon").css("color", "white");
						// $(".action.showcart:before").css("color", "white");
					}
				});
			}
			/* Footer */
...
function calAndSetHeightHeaderFooter() {
    require([
		...
		if(window.matchMedia("(max-width: 767.89px)").matches){
			$heightHeader += 42;
		}
...
require( ['jquery','mage/translate'], function($) {
	...
	if($(this).width() <= 767){
		...
		$(document).on("click",".block-title.filter-title",function(e){
